...
| 欢迎页面 | 地图主页 |
| :---: | :---: |
| ![Welcome](docs/welcome.webp) | ![Map](docs/map.webp) |

## 技术栈与改进

相比于原版项目，我们在现代化进程中做了如下改进：

- **UI 框架**：全面迁移至 `com.google.android.material:material:1.12.0` (MD3)。
- **地图引擎**：全面替换百度地图，采用 **OSMDroid**，并通过自定义颜色矩阵实现了低功耗、高对比度的地图暗黑模式。
- **系统适配**：适配 Android 14 (API 34+) 的安全广播机制、通知权限及前台服务要求。
- **性能优化**：移除了大量的第三方冗余库，优化了 Activity 生命周期管理，确保在主题切换时状态（如缩放级别、坐标）不丢失。
